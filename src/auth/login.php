...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
    header("location: ../pages/about_me/read.php");
    exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        $sql = "SELECT id, username, password, role_id FROM users WHERE username = :username and isdeleted = false";
                ...
                if($stmt->rowCount() == 1){
                    if($row = $stmt->fetch()){
                        ...
                        if(password_verify($password, $hashed_password)){
                            ...
                            $login_err = "Username or password wrong! AQUI";
                        }
                    }
                } 
                else{
